...
import { Search, Edit3, Gift, Star, ChevronDown } from "lucide-react";
...
export default function CSUsers() {
  ...
  const [couponModal, setCouponModal] = useState<{ isOpen: boolean, user: any, code: string, discount: string }>({ isOpen: false, user: null, code: "WELCOMETH", discount: "10" });
  ...
  return (
    <CSLayout>
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center mb-6">
        <h1 className="text-2xl font-bold text-stone-800">会员与用户管理</h1>
              ...
              {isLoading ? (
                ...
                <tr key={user.id} className="hover:bg-stone-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right flex items-center justify-end gap-2">
                    ...
                    <Button variant="outline" size="sm" className="text-emerald-600 border-emerald-200 hover:bg-emerald-50" onClick={() => setCouponModal({ isOpen: true, user, code: "WELCOMETH", discount: "10" })}>
                      <Gift className="w-4 h-4 mr-1" /> 发券
                    </Button>
                  </td>
                </tr>
              ))}
      ...
      {pointsModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <Card className="w-full max-w-md p-6 bg-white shadow-xl rounded-xl">
            <h2 className="text-xl font-bold mb-4">手动调整BL coin</h2>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <Input placeholder="例如: 客服补偿、线下充值等" value={pointsModal.reason} onChange={e => setPointsModal({...pointsModal, reason: e.target.value})} />
              </div>
            </div>
      ...
      {couponModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <Card className="w-full max-w-md p-6 bg-white shadow-xl rounded-xl">
            <h2 className="text-xl font-bold mb-4">发放个人优惠券</h2>
            <div className="mb-4 text-sm text-stone-600">
              将优惠券发放到顾客 <strong className="text-stone-800">{couponModal.user?.name}</strong> 的账户中。
            </div>
            <div className="space-y-4 mb-6">
              <div>
